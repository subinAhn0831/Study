...
- 열거형은 toString()을 재정의해서 toString() 하면 name() 값이 출력
- 각 열거형이 개별적 주소값을 가짐(인스턴스가 각각 생성됨)
- 열거형을 static import 하면 ‘BASIC ‘ 이렇게 이름만으로 코드 작성 가능
- 제공하는 메서드 있음(필요시 구글검색)
...
- 정적 중첩 클래스
    - 외부 클래스와 , 내부 정적 클래스는 메모리 구조상 아예 관련이 없음
    - 논리적 구조로 봤을 때, 한 클래스가 한 클래스 안에서만 쓰이는 특성을 가진 경우에만 사용함
    - 대부분의 경우는 중첩하는 것 보다 별도의 클래스 파일로 생성함
    - 정적 중첩 클래스의 경우, 외부 클래스의 private static 멤버에 접근 가능
    - 정적 중첩클래스는 외부에서, 외부클래스.내부클래스 의 형태로 사용 가능
    - 정적 중첩 클래스에 private을 붙히면 외부에서도 사용 불가

```java
참고
static 으로 선언했을 경우, static은 static만 호출 가능!
static 으로 생성하는 경우는 보통 특성이 유틸이나 헬퍼 일 때
static은 인스턴스에 속하지 않는, 객체 상태에 종속되지 않음 -> 메모리 상 별도로 할당됨
```

### 내부클래스

- 내부클래스는 바깥인스턴스에 개념상 속해있다고 보면 됨